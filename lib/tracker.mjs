...
const log = content => lib.utilitas.log(content, import.meta.url);
const hostname = 'localhost';
const statusKey = 'pt_status';
const renderHost = host => /^\:\:/.test(host) ? hostname : host;
const getServer = () => _server;
const getStatus = () => lib.memory.get(statusKey);
...
const init = async () => {
    ...
    _server.listen(config.trackerPort, hostname, () => log(`Server is up!`));
};

const checkStatus = async () => {
    if (!_server) { return await init(); }
    ...
    await lib.memory.set(statusKey, _status);
    return _status;
};
