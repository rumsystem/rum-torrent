...
const init = async (options) => {
    _server = new Server({
        ...
        filter: async (infoHash, params, callback) => {
            ...
            try { options?.auth && await options.auth(infoHash, params); } // OK
            catch (error) { callback(error); } // new Error('disallowed')  // Blocked
            callback(null);
        },
        ...options || {},
    });
    ...
    _server.listen(
        { http: options?.httpPort || httpPort, udp: options?.udpPort || udpPort },
