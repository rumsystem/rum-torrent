...
import { announce, localAnnounce } from './config.mjs';
...
const [BINARY, suffix] = ['binary', 'torrent'];
const TORRENT_ENCODING = { encoding: 'NULL' };
...
const parseTorrentFromFile = async (file) => {
    const torrent = await storage.readFile(file, TORRENT_ENCODING);
    return await parseTorrent(torrent);
};
...
const createTorrentFile = async (content, options) => {
    const { torrent, details } = await createTorrent(content, options);
...
const seed = async (input, options) => {
    ...
    options = {
        ...
        announce: localAnnounce, ...options || {},
    };

    if (/^magnet:/ig.test(input)) {
        // for (let i in announce) {
        //     input.indexOf(announce[i]) === -1
        //     announce[i]
        // }
    } else if (/\.torrent$/ig.test(input)) {
        const details = await parseTorrentFromFile(input);
        console.log(details);
    }
