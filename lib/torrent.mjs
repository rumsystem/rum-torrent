...
const parseTorrentFromFile = async file => parseTorrent(
...
const createTorrent = async (content, options) => {
    ...
    const details = parseTorrent(torrent);
...
const seed = async (input, options) => {
    ...
    if (/^magnet:/ig.test(input)) {
        ...
        for (let [key, value] of search) {
            utilitas.insensitiveCompare(key, 'tr') && curAnc.push(value);
        }
    } else if (/\.torrent$/ig.test(input)) {
        curAnc = (await parseTorrentFromFile(input)).announce || [];
    }
    curAnc.map((v, k) => curAnc[k] = utilitas.ensureString(v, { case: 'LOW' }));
    announce.map((v, k) => {
        let check = false;
        for (let x of curAnc) {
            if (x.includes(v)) {
        ...
        check || options.announce.push(localAnnounce[k]);
    });
