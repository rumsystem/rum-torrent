...
import { getAnnounce, publish as _publish } from './auth.mjs';
...
const publish = async (input, privateKey, options) => {
    ...
    return await _publish(parsed.infoHash, privateKey);
};
...
const seed = async (input, options) => {
    ...
    let parsed, nftadd = options?.nftadd, token;
    if (/^magnet:/ig.test(input)) {
        parsed = await parseTorrent(input);
        if (/^.*na=([0-9a-z]*).*$/ig.test(input)) {
            nftadd = input.replace(/^.*na=([0-9a-z]*).*$/ig, '$1');
        }
        if (/^.*tk=([0-9a-z]*).*$/ig.test(input)) {
            token = input.replace(/^.*tk=([0-9a-z]*).*$/ig, '$1');
        }
    } else if (/\.torrent$/ig.test(input)) {
        parsed = await parseTorrentFromFile(input);
    } else {
    ...
    options.announce = [...new Set(getAnnounce([
        ...parsed.announce, ...options.announce
    ], { ...options || {}, token, btih: parsed.infoHash, nftadd }))];
