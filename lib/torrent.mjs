...
import { localAnnounce } from './config.mjs';
...
const ignore = new Set([
    'blockedPeers', 'downloaded', 'hotswaps', 'infoHash', 'peers',
...
const createTorrent = async (content, options) => {
    ...
    const torrent = await pmfCreateTorrent(content, {
        announceList: options?.announce || [localAnnounce],
...
const getClient = async (options) => {
    if (!_client) {
        _client = new webTorrent({
            ...
            peerId: createPeerId(),                // String | Buffer, Wire protocol peer ID (default=randomly generated)
