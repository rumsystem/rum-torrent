...
const splitLine = () => utilitas.fullLengthLog('', { padding: '-' });
const renderSplit = ' : ';
...
const createTorrent = async (content, options) => {
    ...
    const torrent = await pmfCreateTorrent(content, {
        announceList: options?.announce || [config.localAnnounce], comment, ...options || {}
    });
...
const render = (status) => {
    console.clear();
    // console.log(_client.torrents[0].info);
...
const seed = async (input, options) => {
    ...
    options = {
        announce: config.localAnnounce, path: fromMeta ? storage.getTempPath({ sub: input }) : input, ...options || {},
    };
